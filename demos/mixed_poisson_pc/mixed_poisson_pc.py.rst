...
Strategies for solving the mixed Poisson problem
================================================

Introduction
------------

In this demo, we will discuss strategies for solving the mixed
formulation of the Poisson equation introduced in
:doc:`/demos/poisson_mixed.py`.  For ease of exposition, we repeat the
formulation here.  Starting from the primal form of the Poisson
equation, :math:`\nabla^2 u = -f`, we introduce a vector-valued flux,
:math:`\sigma = \nabla u`.  The problem then becomes to find :math:`u`
and :math:`\sigma` in some domain :math:`\Omega` satisfying
...
along with a function to hold the forcing term, living the
discontinuous space. ::
...
cases later.  The ``aP`` function takes one argument, the
:class:`~.FunctionSpace` defining the space and returns a
bilinear form suitable for assembling into an operator.  Obviously we
only do so if ``aP`` is provided. ::
...
to a dict of parameters.  These parameters are passed directly to
PETSc_, and the form of them is described in more detail in
...
preconditioner.  The simple setup can be driven using only solver
options.  As before, we will use restarted GMRES solved to a relative
tolerance of 1e-8::
...
but now we want a ``fieldsplit`` preconditioner ::
...
inverting :math:`A` and :math:`S` to check that we converge in one
FIXME (?)  GMRES iterates (as theory suggests we should) ::
...
As we increase the number of mesh elements, the solver takes more and
more iterations, which means that we take longer and longer to solve
the problem as the mesh is refined.
...
This is much better, we obtain inner iteration counts as follows 
...
try drop some terms in the factorisation of :math:`P`, by adjusting
...
:math:`Sp`.  We then tell PETSc about this operator by providing
asking that the Schur complement be preconditioned with a ``user``
operator.  The parameter set we will use is therefore ::
